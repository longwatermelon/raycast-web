...
struct Player *player_init(SDL_Point pos, float angle, SDL_Renderer *rend)
{
    ...
    if (!(self->shot_texture = IMG_LoadTexture(rend, "assets/images/gun_shoot.png")) ||
        !(self->gun_texture = IMG_LoadTexture(rend, "assets/images/gun.png")) ||
        !(self->knife_texture = IMG_LoadTexture(rend, "assets/images/knife.png")))
    {
...
struct Entity *player_attack(struct Player *self, struct Entity **entities, size_t entities_size, struct Map *map)
{
    ...
    case WEAPON_KNIFE:
    {
        ...
        /* if (ret) */
        /*     /1* audio_play_sound("res/sfx/stab.wav"); *1/ */
        /* else */
